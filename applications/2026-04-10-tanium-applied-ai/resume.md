...
Self-taught AI engineer who ships production AI pipelines and builds primarily by directing coding agents. I've shipped four projects in 18 months, including a live application with a multi-stage vision pipeline deployed on GCP. Strongest at applied LLM work: prompt engineering, structured outputs, tool calling, human verification gates, and cost-tiered model selection. Looking for an applied AI role where building and deploying AI agents is the core job.
...
- Built a 4-stage AI pipeline on the OpenAI Responses API for digitizing handwritten letters: GPT-5.4 Vision for transcription with Sharp image preprocessing, strict structured outputs with Zod validation for metadata extraction, tool calling for entity resolution, and cost-tiered model selection (GPT-5.4 for complex documents, GPT-5.4-mini for simpler ones).
- Designed a human verification gate between transcription and metadata extraction. Guillemet tagging marks uncertain text so reviewers focus where the model is least confident. Per-document-type prompts handle letters, envelopes, telegrams, and photos differently.
...
- Full end to end pipeline: audio to spectrogram to CNN inference to piano roll to MIDI to sheet music via MuseScore. Trained on GCP GPU VMs.
...
**AI / ML:** OpenAI Responses API (GPT-5.4/5.4-mini, strict structured outputs, tool calling, vision), HITL pipeline design, prompt engineering (zero-shot, few-shot, chain-of-thought), cost-tiered model selection, PyTorch (CNN training on real datasets), NLP text extraction and entity resolution, librosa, spectrogram processing, audio fingerprinting, GCP GPU training
